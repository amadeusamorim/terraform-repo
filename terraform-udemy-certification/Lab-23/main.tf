provider "aws" { // Por default, o provider utilizado sempre será esse
  region = "us-west-1"
}

provider "aws" {
  region = "eu-south-1"
  alias  = "EUROPE" // Como tem outro provedor com o mesmo nome, precisa do alias
}
...
data "aws_ami" "europe_latest_ubuntu20" {
  provider    = aws.EUROPE // Para diferenciar do provider padrão é ncessário repassar uma variável de provider com os alias equivalentes
  owners      = ["099720109477"]
  most_recent = true
  filter {
    name   = "name"
