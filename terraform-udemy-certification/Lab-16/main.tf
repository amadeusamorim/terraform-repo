...
# locals definem as variáveis locais
locals {
  Region_fullname   = data.aws_region.current.description
  Number_of_AZs     = length(data.aws_availability_zones.available.names) // Lenght da lista de nomes das Availabilities Zones
...
resource "aws_vpc" "main" {
  ...
  tags = {
    Name           = "My VPC"
    Region_Info_v1 = local.Region_Info // Utilizando as variáveis locais
